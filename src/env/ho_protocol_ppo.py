...
class AbstractTask(ABC):
    ...
    def __init__(
        self, name: str = None, debug: bool = False, verbose: int = None
    ) -> None:
        self.name = name
        self.debug = debug
    ...
    def debug_msg(self, msg: str, lvl: int = None, tic: int = None) -> None:
        """Print debug message."""
        if self.debug and lvl <= self.verbose:
...
class SyncSignal(AbstractTask):
    ...
    def check_out_of_sync(self, sinr_db: np.ndarray, bs: int, tic: int) -> None:
        ...
        # print(f"sinr_db: {sinr_db[bs]}, best sinr: {sinr_db.max()}")
...
class RadioResourceControl(AbstractTask):
    ...
    def update_ncell(self, ncell: int, tic: int) -> None:
        """RRC measurement."""
        if ncell != self.pcell:
            self.ncell = ncell
    ...
    def update_tcell(self, tcell: int) -> None:
        """Update target cell."""
        self.tcell = tcell
    ...
    def get_initial_cell(self, rsrp_db: np.ndarray) -> int:
        """Get initial cell."""
        self.pcell = np.argmax(rsrp_db)
        return self.pcell

    def reconnect(self, target_cell: int, tic: int) -> None:
        """Reconnect to suitable cell."""
        if not np.isnan(target_cell):
...
class GeneralCounter(AbstractTask):
    ...
    def reset(self, tic: int = None, reset_all: bool = False) -> None:
        """Reset task."""
...
class HOProcedurePPO:
    ...
    def __init__(self, protocol_config: "Config") -> None:
        ...
        self.permit_ho_prep_abort = False
    ...
    def _ho_preparation_handler(self, tic: int) -> None:
        ...
        if self.rrc.is_connected:  # Connected to cell
            ...
            # Abort HO preparation and stay in current cell (if allowed)
                ...
                self.cntr["ho_prep"].reset(tic)
                # Update RRC nCell
                self.rrc.update_ncell(np.nan, tic)
            else:
                pass
    ...
    def _mean_capacity(self) -> float:
        ...
        return np.mean(np.log2(1 + sinr_t_lin)) if len(sinr_t_lin) > 0 else 0
    ...
    def get_pcell(self) -> int:
        """Get primary cell."""
        return self.rrc.pcell

    def get_tcell(self) -> int:
        """Get target cell."""
    ...
    def get_stats_dict(self) -> dict[str, int | float]:
        """Return empty dict."""
        return {
    ...
    def get_statistics(self, sinr_db_mat: np.ndarray) -> dict[str, int | float]:
        """Get statistics."""
