...
class AbstractTask(ABC):
    ...
    def __init__(
        self, name: str | None = None, debug: bool = False, debug_lvl: int | None = None
    ) -> None:
        self.name = name
        self.debug = debug
    ...
    def debug_msg(
        ...
        """Print debug message."""
        if self.debug and lvl is not None and lvl <= self.debug_lvl:
...
class RadioResourceControl(AbstractTask):
    ...
    def cell_detected(self) -> bool:
        ...
        if self.ncell is None:
            self.debug_msg("No suitable cell detected.", 3)
            return False
        self.debug_msg(f"Suitable cell detected: PCI {self.ncell}", 3)
        return True
    ...
    def is_connected(self) -> bool:
        ...
        if self.pcell is None:
            self.debug_msg("Not connected to any cell.", 3)
            return False
        self.debug_msg(f"Connected to cell: PCI {self.pcell}", 3)
        return True
...
class HOEventHandler(AbstractTask):
    ...
    def __init__(self, debug: bool = False, **kwargs: Any) -> None:
        if kwargs.get("debug_lvl") is None and isinstance(kwargs["debug_lvl"], int):
            ...
            debug_lvl = None
        super().__init__("HOEventHandler", debug=debug, debug_lvl=debug_lvl)
...
class GeneralCounter(AbstractTask):
    ...
    def reset(self, tic: int | None = None) -> None:
        """Reset task."""
        if self.flag_is_reset:
            return
        super().reset(tic)
